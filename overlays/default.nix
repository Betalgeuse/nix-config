...
final: prev: {
  ...
  tmux-file-picker = prev.callPackage ../pkgs/tmux-file-picker.nix { };
  ...
  workmux = inputs.workmux.packages.${final.system}.default.overrideAttrs (old: {
    patches = (old.patches or [ ]) ++ [
      ../pkgs/workmux/fix-bash-completion-panic.patch
    ];
  });
}
